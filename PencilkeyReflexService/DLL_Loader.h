...
namespace DLLDynamicLoading {
	...
	public:
		...
		int load_dll_module() {
			...
			if (_dllModule == nullptr) {
				...
				report_error(L"Failed to load DLL: " + _dllPath + L" with error code: " + std::to_wstring(_load_status_code));
				return _load_status_code;
			}
			_load_status_code = 0;
			return 0;
	...
	private:
		...
		static void receiveAndRedirectLog(const wchar_t* log) {
			if (_instance) {
				std::wstring wlog = std::wstring(log);
				_instance->report_error(wlog);
			}
		}
	...
	public:
		...
		__int64 CommandSendGetPenConnectStatus() {
			if (_commandSendGetPenConnectStatus) {
				...
				report_error(L"Function CommandSendGetPenConnectStatus is not loaded");
				return -1;
			}
		}
		...
		__int64 CommandSendGetPenSerialNo() {
			if (_commandSendGetPenSerialNo) {
				...
				report_error(L"Function CommandSendGetPenSerialNo is not loaded");
				return -1;
			}
		}
		...
		__int64 RegisterCallBackUpdatePenSerialNo(std::function<void(const wchar_t*)> callback) {
			if (_registerCallBackUpdatePenSerialNo) {
				...
				report_error(L"Function RegisterCallBackUpdatePenSerialNo is not loaded");
				return -1;
			}
		}
		...
		__int64 RegisterCallBackUpdatePenConnectStatus(std::function<void(int)> callback) {
			if (_registerCallBackUpdatePenConnectStatus) {
				...
				report_error(L"Function RegisterCallBackUpdatePenConnectStatus is not loaded");
				return -1;
			}
		}
		...
		__int64 CommandSendPenCurrentFunc(UINT8 val) {
			if (_commandSendPenCurrentFunc) {
				...
				report_error(L"Function CommandSendPenCurrentFuncType is not loaded");
				return -1;
			}
		}
		...
		__int64 CommandSendSetPenKeyFunc(UINT8 val) {
			if (_commandSendSetPenKeyFunc) {
				...
				report_error(L"Function CommandSendSetPenKeyFunc is not loaded");
				return -1;
			}
		}
	};
