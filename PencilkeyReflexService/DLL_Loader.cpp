...
namespace DLLDynamicLoading {
	...
	void HuaweiPencilServiceDLL::load_functions() {
		...
		if (!_commandSendGetPenConnectStatus) {
			report_error(L"Failed to load function: CommandSendGetPenConnectStatus");
		}
		...
		if (!_commandSendGetPenSerialNo) {
			report_error(L"Failed to load function: CommandSendGetPenSerialNo");
		}
		...
		if (!_registerCallBackUpdatePenSerialNo) {
			report_error(L"Failed to load function: RegisterCallBackUpdatePenSerialNo");
		}
		...
		if (!_registerCallBackUpdatePenConnectStatus) {
			report_error(L"Failed to load function: RegisterCallBackUpdatePenConnectStatus");
		}

		_registerLogFunc = LoadFunction<RegisterLogFuncType>("RegisterLogFunc");
		if (!_registerLogFunc) {
			report_error(L"Failed to load function: RegisterLogFunc");
		}
		else {
		...
		if (!_commandSendPenCurrentFunc) {
			report_error(L"Failed to load function: CommandSendPenCurrentFunc");
		}
